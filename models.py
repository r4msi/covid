...
class Models:
    ...
    def fit_ols(self):
        ...
        new = pd.concat((self.dt[["fallecimientos","fecha"]],predictions), axis=0)
        ...
        predictions["Error"] = 0
        p=pd.concat([predictions.reset_index(drop=True), historic])
    ...
    def fit_sarimax(self):
        ...
        sum = sarimax.summary()
        predictions = pd.DataFrame(
            sarimax.predict(6, self.forecast.drop(["fecha","days"], axis=1))
        )
